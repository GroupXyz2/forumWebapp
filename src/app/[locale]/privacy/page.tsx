...
export default function PrivacyPage({ params }: PrivacyPageProps) {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const response = await fetch(`/${locale}/api/admin/settings?key=page_privacy`);
        ...
        if (response.ok) {
          ...
          if (data && data.settings && data.settings.length > 0) {
            ...
            setContent(tr(locale, 'privacyPlaceholder', 'common'));
          }
        } else {
          setContent(tr(locale, 'privacyPlaceholder', 'common'));
        }
  ...
  return (
    <main className="flex flex-col items-center justify-between py-8">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-2">{tr(locale, 'privacy', 'common')}</h1>
